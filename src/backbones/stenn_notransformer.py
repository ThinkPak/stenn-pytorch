...
class STENN_NoTransformer(nn.Module):
    def __init__(
            self,
            input_dim,
        ...
        super(STENN_NoTransformer, self).__init__()
        encoder_widths = [input_dim] + encoder_widths
        self.n_stages = len(encoder_widths)
        # 卷积块
        self.conv_blocks = nn.ModuleList(
            SameConvBlock(
                d_in=sum(encoder_widths[0:i + 1]),
    ...
    def forward(self, input, batch_positions):
        ...
        # 空洞卷积块（四个）
        for i in range(self.n_stages - 1):
            out = self.conv_blocks[i].smart_forward(torch.cat(feature_maps[0:i + 1], dim=-3))
